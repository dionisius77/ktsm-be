...
import { AuthServiceClient } from "@app/entities/grpc-interfaces/auth-grpc.interface";

@Injectable()
export class BranchAuthHelper implements OnModuleInit {
  ...
  public async validateBranch(decoded: any): Promise<Branch> {
    try {
      return await this.authService.getBranch({ id: decoded.id }).toPromise();
    } catch (error) {
      console.error(error);
    }
  }
  ...
  // Generate JWT Token
  public generateBranchToken(user: Branch): string {
    return this.jwt.sign({
      id: user.id,
      email: user.email,
      // TODO: change this to branch id parkingId: user.parkingId,
    });
  }
  ...
  public async validate(token: string): Promise<boolean | never> {
    ...
    const user = await this.validateBranch(decoded);
    if (!user) {
